...
set -euo pipefail
#####################################################################
#  Liam’s Zsh Bootstrap (2025-08-04, alias-finder removed)
#####################################################################
...
cat > "$HOME/.zshrc" <<"EOF"
##############################################################################
#  Liam’s Optimised .zshrc  – 2025-08-04
##############################################################################
...
# Stub widgets for fast-syntax-highlighting
...
# Manually load order-sensitive plugins
...
setopt APPEND_HISTORY INC_APPEND_HISTORY SHARE_HISTORY \
       ...
       HIST_FIND_NO_DUPS HIST_IGNORE_SPACE EXTENDED_HISTORY
...
🗂️  ALIASES
  ...
  update-all        – full package upgrade
...
ALIAS
      ...
      2)
        cat <<'CMDS'
🖥️  SYSTEM
  btop, bottom, duf  – monitors
🌐  NETWORK
  bandwhich, iperf3, ncat
🔎  SEARCH
  rg, fd, fzf
📁  FILES
  nnn, bat, eza
...
if [ "$SHELL" != "$(command -v zsh)" ]; then
  ...
  INFO "Default shell changed (log out/in to activate)"
fi

STEP "Bootstrap complete! Launch a new terminal or run: exec zsh"
